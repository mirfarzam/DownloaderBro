...
def check_mentions(api, keywords, since_id):
    ...
    for tweet in tweepy.Cursor(api.mentions_timeline, since_id=since_id).items():
        ...
        if tweet.in_reply_to_status_id is None:
            continue
        main = (api.statuses_lookup([tweet.in_reply_to_status_id], include_entities=True ))
        if len(main) > 0:
            main = main[0]
        else:
            api.update_status(f'@{tweet.user.screen_name} Hi Bro! I cannot find the replay', in_reply_to_status_id = tweet.id_str) 
            contin
        final_video = None
        try :
            ...
            #    final_video = max(MyCount, key=int)
               if maxURL is not None:
                   api.update_status(f'@{tweet.user.screen_name} Hi Bro! this is the Link {maxURL}', in_reply_to_status_id = tweet.id_str) 
                                   #    direct_message = api.send_direct_message(user=tweet.id_str, text=f'{tweet.user.screen_name} Hi Bro! this is the Link {maxURL}')
